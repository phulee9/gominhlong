...
BẢN SỬA (đánh dấu [FIX 1]..[FIX 10]):
  ...
  [FIX AUTO-LOGIN] Tự đăng nhập lại KHÔNG cần mật khẩu/OTP khi session
          hết hạn — dựa trên cơ chế "thiết bị tin cậy" của MISA (profile
          này đã từng đăng nhập đầy đủ ít nhất 1 lần; trạng thái tin cậy
          lưu trong localStorage của chính profile, sống lâu hơn session
          cookie hay chết theo browser theo thiết kế). Xem
          try_auto_login/try_skip_verification/ensure_logged_in. Chỉ khi
          gặp Ô MẬT KHẨU THẬT (thiết bị mất trạng thái tin cậy — hiếm)
          mới thật sự dừng lại, yêu cầu --setup thủ công.
"""
import argparse
import asyncio
import re
...
# ĐÃ ĐỔI sang nền tảng MISA AMIS Platform mới (xem MISA_LOGIN_URL bên
# dưới) — profile này phải đã được đăng nhập đầy đủ (mật khẩu + OTP) và
# tick "ghi nhớ thiết bị" ÍT NHẤT 1 LẦN trên chính nền tảng mới, nếu
# không sẽ luôn bị yêu cầu OTP mà script không tự vượt qua được.
# ⚠️ TUYỆT ĐỐI không mở lại thư mục này bằng playwright codegen hay chạy
# tay sau khi đã dùng cho Airflow — xem cảnh báo đầu file.
PROFILE_DIR = Path(r"D:\Source\Automation\misa_automation_profile_new")
...
# [FIX AUTO-LOGIN] Định danh đăng nhập + selector cho luồng tự đăng nhập
# lại KHÔNG cần mật khẩu (cơ chế "thiết bị tin cậy" — xem
# try_auto_login/ensure_logged_in). Lấy nguyên văn từ Playwright codegen
# thật. THÔNG TIN NHẠY CẢM tương tự COOKIES_FILE — không copy ra ngoài.
...
# Số giây tối đa thử tự đăng nhập lại tại chỗ khi phát hiện session hết
# hạn — chạy unattended theo lịch, không có ai chờ, nên không cần đợi
# lâu như dang_nhap_misa.py (script dành cho khách tự double-click).
AUTO_RELOGIN_TIMEOUT_SECONDS = 45
...
# [MIGRATE AMIS PLATFORM] Nền tảng cũ (aspapp.misa.vn/id.misa.vn) đã bị
# MISA khoá cho tài khoản này — công ty kiểm toán quản lý tài khoản đã
# chuyển sang cổng đăng nhập hợp nhất "MISA AMIS Platform". URL báo cáo
# giờ cũng theo dạng trực tiếp app.MISA_APP_BASE_URL/asp/<TenReport> (đã
# xác nhận qua thực tế: /asp/Customer), KHÁC hẳn cấu trúc app.RP/... cũ —
# TOÀN BỘ report_url trong REPORTS_TO_DOWNLOAD bên dưới còn là URL CŨ,
# CHƯA cập nhật, cần dò lại từng report trước khi chạy thật.
MISA_LOGIN_URL = "https://amisapp.misa.vn/login/"
MISA_APP_BASE_URL = "https://actasp.misa.vn"
# MISA_APP_BASE_URL = "https://amisapp.misa.vn"  # domain mới thay vì actasp.misa.vn
...
REPORTS_TO_DOWNLOAD: list[dict] = [
    ...
    },
    {
        ...
        "export_selector": DETAIL_REPORT_EXPORT_SELECTOR,
        "export_hover_selector": DETAIL_REPORT_EXPORT_HOVER_SELECTOR,
        "wait_before_export_ms": 5_000,   # <-- thêm dòng này, chỉnh số giây tùy ý
    },
    {
        ...
        "download_timeout_ms": 100_000,
        "wait_before_export_ms": 35_000,   # <-- thêm dòng này, chỉnh số giây tùy ý
    },
    {
...
async def try_skip_verification(context) -> int:
    """[FIX AUTO-LOGIN] Quét MỌI tab đang mở, bấm nút 'Bỏ qua, tiếp tục
    làm việc' nếu đang hiển thị (màn hình xác thực thiết bị MISA chèn sau
    khi bấm "Đăng nhập"). Selector lấy từ Playwright codegen thật. Trả về
    số lần vừa bấm được (0 = bình thường, không phải lỗi)."""
    clicked = 0
...
async def try_auto_login(page) -> str:
    """[FIX AUTO-LOGIN] Tự điền định danh đăng nhập + bấm 'Đăng nhập' —
    KHÔNG cần mật khẩu — dựa trên cơ chế "thiết bị tin cậy" của MISA:
    profile này đã từng đăng nhập đầy đủ ít nhất 1 lần, trạng thái tin
    cậy lưu trong localStorage của chính profile (sống lâu hơn session
    cookie). Các bước lấy NGUYÊN VĂN từ Playwright codegen thật:
    ...
    Nếu ô định danh ĐÃ có sẵn giá trị (trình duyệt tự nhớ) thì KHÔNG ghi
    đè — chỉ bấm thẳng "Đăng nhập".

    Trả về:
      'clicked_login'  — vừa điền (nếu cần) + bấm "Đăng nhập" xong, cần
                          đợi vòng lặp sau xem ra màn hình gì tiếp theo.
      'need_password'  — gặp Ô MẬT KHẨU THẬT đang hiển thị — vượt quá
                          khả năng tự động (không lưu mật khẩu), phải
                          dừng lại, cần đăng nhập tay 1 lần.
      'no_action'       — chưa có gì để làm ở bước này — thử lại sau.
    """
    try:
    ...
    try:
        ...
        pass
    return "no_action"
async def ensure_logged_in(context, page) -> None:
    """[FIX AUTO-LOGIN] Nếu session đã hết hạn, thử TỰ đăng nhập lại tại
    chỗ (không cần người): lặp điền định danh -> bấm Đăng nhập -> bấm bỏ
    qua xác thực, trong tối đa AUTO_RELOGIN_TIMEOUT_SECONDS giây. Đây là
    bước để download_reports() chạy được KHÔNG người giám sát theo lịch.

    Chỉ raise SystemExit nếu sau thời gian trên vẫn hết hạn (gặp Ô MẬT
    KHẨU THẬT — thiết bị mất trạng thái tin cậy, cần khách đăng nhập tay
    1 lần bằng dang_nhap_misa.py hoặc --setup; giới hạn không thể vượt
    qua vì không lưu mật khẩu/OTP)."""
    if not await is_session_expired(page):
        return
    print("    [auto-relogin] session hết hạn — đang thử tự đăng nhập lại tại chỗ...")
    deadline = time.monotonic() + AUTO_RELOGIN_TIMEOUT_SECONDS
    attempted_urls: set[str] = set()
    while time.monotonic() < deadline:
        skipped = await try_skip_verification(context)
        if skipped:
            print(f"    [auto-relogin] đã bấm 'Bỏ qua xác thực' ({skipped} tab).")
        for pg in list(context.pages):
            key = f"{id(pg)}::{pg.url}"
            ...
            if result == "clicked_login":
                ...
                print("    [auto-relogin] đã tự điền định danh + bấm 'Đăng nhập'.")
            elif result == "need_password":
                raise SystemExit(
                    "Session hết hạn và gặp Ô MẬT KHẨU thật — thiết bị mất "
                    "trạng thái tin cậy, không tự đăng nhập tiếp được. Cần "
                    "đăng nhập tay 1 lần: chạy python dang_nhap_misa.py "
                    "(hoặc python misa_report_downloader_win.py --setup)."
                )
        if not await is_session_expired(page):
            print("    [auto-relogin] đã đăng nhập lại thành công.")
            return
        await asyncio.sleep(1.5)
    raise SystemExit(
        f"Session hết hạn — đã thử tự đăng nhập lại trong "
        f"{AUTO_RELOGIN_TIMEOUT_SECONDS}s nhưng không thành công. Cần kiểm "
        f"tra lại thủ công (python misa_report_downloader_win.py --show)."
    )
...
async def set_date_range(page, from_date: str, to_date: str,
    ...
    if not panel_already_open:
        ...
        MAX_PARAM_BUTTON_ATTEMPTS = 3
        PARAM_BUTTON_WAIT_MS = 2000
        PARAM_BUTTON_RETRY_GAP_MS = 1000
        for attempt in range(MAX_PARAM_BUTTON_ATTEMPTS):
            await dismiss_message_overlay(page)   # <-- thêm dòng này, đóng banner nếu có
            try:
                btn = page.get_by_role("button", name="Chọn tham số")
                await btn.first.wait_for(state="visible", timeout=PARAM_BUTTON_WAIT_MS)
                await btn.first.click(timeout=3000)
                print("    [date] đã bấm 'Chọn tham số'.")
                await page.wait_for_timeout(500)
                break
            except Exception:
                if attempt < MAX_PARAM_BUTTON_ATTEMPTS - 1:
                    print(f"    [date] chưa thấy nút 'Chọn tham số' (lần {attempt+1}/{MAX_PARAM_BUTTON_ATTEMPTS}) — thử đóng banner rồi thử lại...")
                    await page.wait_for_timeout(PARAM_BUTTON_RETRY_GAP_MS)
                else:
                    print("    [date] không thấy nút 'Chọn tham số' sau khi đã thử đóng banner — panel có thể đã mở sẵn.")
    else:
...
async def click_export_and_download(
    ...
    wait_before_export_ms: int = 0,   # <-- thêm dòng này
):
    ...
    # ── TRƯỜNG HỢP B: bấm export tải luôn (hầu hết report) ───────────────
    if wait_before_export_ms > 0:
        print(f"    [export] chờ {wait_before_export_ms/1000:.0f}s trước khi bấm export...")
        await page.wait_for_timeout(wait_before_export_ms)
...
async def _already_in_company_page(page) -> bool:
    """[FIX PROFILE-REMEMBER] Profile đã nhớ công ty đã chọn trước đó
    -> MISA có thể nhảy thẳng vào trang công ty, bỏ qua màn chọn công ty."""
    if "actasp.misa.vn" in page.url:
        try:
            text = await page.content()
            return COMPANY_NAME_HINT.upper() in text.upper()
        except Exception:
            return False
    return False

async def login_and_select_company(context):
    """Đăng nhập + chọn công ty, trả về page TAB công ty thật (actasp.misa.vn).

    [CẬP NHẬT SAU MIGRATE AMIS PLATFORM] Domain đăng nhập/chọn công ty mới
    là amisapp.misa.vn/asp/Customer (KHÔNG còn Account/Join như domain cũ).
    Xác nhận qua Playwright codegen thật:
        page.goto("https://amisapp.misa.vn/asp/Customer")
        with page.expect_popup() as page1_info:
            page.locator(
                ".grid-list.has-resize.dataTable.no-footer.DTFC_Cloned "
                "> tbody > tr > .text-right > .grid-inline-action > .il-text"
            ).first.click()
        page1 = page1_info.value   # <-- đây là tab actasp.misa.vn thật

    Bấm vào đúng DÒNG công ty khớp COMPANY_NAME_HINT (không dùng .first
    mù quáng — rủi ro chọn nhầm công ty nếu danh sách có nhiều dòng),
    tương tự cách làm cũ với click_company_card_button.
    """
    page = await context.new_page()
    await page.goto(
        "https://amisapp.misa.vn/asp/Customer",
    ...
    # Chờ bảng danh sách công ty render xong
    grid_row_selector = (
        ".grid-list.has-resize.dataTable.no-footer.DTFC_Cloned > tbody > tr"
    )
    try:
        await page.locator(grid_row_selector).first.wait_for(
            state="visible", timeout=30_000
        )
    except Exception as e:
        raise RuntimeError(
            f"Không thấy bảng danh sách công ty tại amisapp.misa.vn/asp/Customer "
            f"sau 30s — có thể selector/URL đã đổi tiếp: {e}"
        )

    # Tìm đúng DÒNG khớp COMPANY_NAME_HINT — không dùng .first mù quáng
    target_row = page.locator("tr").filter(has_text=COMPANY_NAME_HINT)
    row_count = await target_row.count()
    if row_count == 0:
        raise RuntimeError(
            f"Không tìm thấy dòng công ty khớp '{COMPANY_NAME_HINT}' trong "
            f"danh sách tại amisapp.misa.vn/asp/Customer."
        )

    action_selector = ".text-right > .grid-inline-action > .il-text"

    async with page.expect_popup(timeout=30_000) as popup_info:
        await target_row.first.locator(action_selector).click(
            force=True, timeout=10_000
        )
    company_page = await popup_info.value

    # Chờ tab mới rời khỏi trang callback/loading, xác nhận vào đúng actasp.misa.vn
    ...
    try:
        await company_page.wait_for_load_state("domcontentloaded", timeout=60_000)
    except Exception:
        pass
    ...
    # Xác minh lại đúng công ty — tránh lấy nhầm dữ liệu công ty khác
    ...
    if COMPANY_NAME_HINT.upper() not in page_text.upper():
        raise RuntimeError(
            ...
            f"sau khi chọn — DỪNG LẠI, không tiếp tục tải report."
        )
...
async def process_one_task(
    ...
    download = await click_export_and_download(
        ...
        wait_before_export_ms=report_cfg.get("wait_before_export_ms", 0),   # <-- thêm dòng này
    )
...
def build_task_list(from_date: str, to_date: str, use_bookmark: bool) -> list[dict]:
    """[NEW LOGIC]
    - bookmark ON:
        + B01_DN, B02_DN, INV_SUMMARY_V2 -> luôn tải 01 THÁNG HIỆN TẠI -> hôm nay
          (1 file duy nhất *_YYYY-MM.xlsx của tháng này, ghi đè mỗi lần chạy;
          KHÔNG còn chốt sổ tháng cũ — DB đã tự rollback theo kỳ khi load).
        + Các report còn lại -> luôn tải 01/01 NĂM HIỆN TẠI -> hôm nay
          (1 file tên chuẩn, DB sẽ TRUNCATE + insert lại toàn bộ).
    - bookmark OFF: giữ nguyên hành vi cũ — 1 file, đúng khoảng
      --from-date/--to-date người dùng truyền vào.
    """
    today = date.today()
    fmt = "%d/%m/%Y"
    tasks: list[dict] = []
    for cfg in REPORTS_TO_DOWNLOAD:
        ...
        if use_bookmark and code in MONTHLY_REPORTS:
            # --- Task 1: THÁNG TRƯỚC — luôn tải trọn vẹn (01 -> ngày cuối tháng trước) ---
            month_from = date(today.year, today.month, 1)
            prev_last_day = month_from - timedelta(days=1)
            prev_y, prev_m = prev_last_day.year, prev_last_day.month
            prev_first_day = date(prev_y, prev_m, 1)
            dest_prev = monthly_path(code, prev_y, prev_m)
            tasks.append({
                "cfg": cfg,
                "from": prev_first_day.strftime(fmt),
                "to": prev_last_day.strftime(fmt),
                "dest": dest_prev,
                "label": f"{code}[{prev_y:04d}-{prev_m:02d}](tháng trước)",
                "kind": "prev",
                "y": prev_y, "m": prev_m,
            })

            # --- Task 2: THÁNG HIỆN TẠI — luôn tải, ghi đè mỗi lần chạy ---
            dest_current = monthly_path(code, today.year, today.month)
            tasks.append({
                "cfg": cfg,
                "from": month_from.strftime(fmt),
                "to": today.strftime(fmt),
                "dest": dest_current,
                "label": f"{code}[{today.year:04d}-{today.month:02d}](hiện tại)",
                "kind": "current",
